...
# compute23's concurrent LAST-stress run over the same NFS mount.
...
# ── Main loop ─────────────────────────────────────────────────────────────────
# Split from the original 25-run job: this host runs a fresh n=5 for
# baseline + last_wsa only (compute25's partial progress on these two
# variants, 2+1 reps, is discarded -- small relative to the multi-hour
# total). last_hfl/mats/mats_raw continue to completion on compute25
# instead (see run_mats_eval_besu_c25_continue.sh), preserving their
# already-collected rep1 data.
echo "======================================================================"
echo "MATS Eval Besu SPLIT | 150 TPS | 120s+300s"
echo "Variants: baseline, last_wsa | Reps 1-5 (fresh)"
echo "Run ID: ${RUN_ID}"
echo "Results: ${RESULTS_DIR}"
echo "======================================================================"
echo "Starting 10 runs..."
...
echo ""
echo "======================================================================"
echo "Split job COMPLETE (baseline/last_wsa both at n=5)."
echo "Results: ${RESULTS_DIR}"
echo "======================================================================"
